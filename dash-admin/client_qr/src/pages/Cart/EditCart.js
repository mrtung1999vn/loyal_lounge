...
function EditCart({ EditData, onHandleEdit, ListTypeCart, handleChangeTypeCart }) {
  const [IDCart, setIDCart] = React.useState(EditData.id_cash)
  const [NameCart, setNameCart] = React.useState(EditData.ten_sp)
  const [Password, setPassword] = React.useState('')
  const [RePassword, setRePassword] = React.useState('')
  const [Address, setAddress] = React.useState('')
  const [PhoneNumber, setPhoneNumber] = React.useState('')
  const [IDTypeCart, setIDTypeCart] = React.useState(EditData.id_loai_sp)
  ...
  const onClickSave = async () => {
    try {
      ...
      const res = await fetch(host.WebDashDanhSachCart, {
        ...
        body: JSON.stringify({ IDCart, Description, TypeStatus, PriceVND, userName })
      })
          ...
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title" id="exampleModalLabel">Cart id {EditData.id_cash} </h5>
