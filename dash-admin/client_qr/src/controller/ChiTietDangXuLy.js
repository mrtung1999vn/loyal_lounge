...
function ChiTietDangXuLy({id_kh,stt,id_don_hang}) {
    ...
    const onClickDatHang = async ()=>{
      try{
          ...
          const JsonData = await response.json()
          if(JsonData.status ===1){
              alert.success("Đặt đơn hàng thành công",{position: positions.BOTTOM_CENTER})
              history.goBack()
          }else{
              alert.error("Đặt đơn hàng thất bại",{position: positions.BOTTOM_CENTER})
          }
                ...
                <div className="invoice p-3 mb-3">
                  ...
                  <address>
                    ...
                    <div className="col-sm-4 invoice-col">
                      ...
                      <b>Mã đơn hàng 1</b><br />
                      <input className="form-control" 
                      value={ma_don}
                      onChange={e=>setma_don(e.target.value)}
                      ></input>
                      {/* <button className="btn btn-primary sm" onClick={()=>onChangeDonHang(ma_don)}>Thêm đơn</button> */}
                      <br />
                        ...
                        <TableBody>
                          {GioHangKHCT.map((x,index)=>(
                           ...
                           <input className="form-control"
                            ...
                            <TableCell   style={{ width: '2rem !important' }}  >{parseInt(KhachHang.map(x => x.ti_gia_tinh)).toLocaleString('vi', { style: 'currency', currency: 'VND' })}</TableCell>
                            <TableCell   style={{ width: '2rem !important' }}  >{
                        ...
                        <table className="table">
                          ...
                          <tr>
                            ...
                            <tr>
                              ...
                              ></input>
                              {
                              parseInt(phi_noi_dia*parseInt(KhachHang.map(x => x.ti_gia_tinh)))
                              }
                              </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                              <th>Thành tiền</th>
                              <td>{
                              parseInt(TienGioHang*parseInt(KhachHang.map(x => x.ti_gia_tinh))
                  ...
                  <div className="row no-print">
                    <div className="col-12">
                      ...
                      onClick={()=>onChangeDonHang(ma_don)}
...
const BtnTachDonKhachHang = ({GioHangKHCT,id_don_hang,onUploadGioHangKH})=>{
  ...
  const onClickTachDonHang = async (GioHangKHCT)=>{
    try{
      ...
      if(GioHangKHCT.length ===1){
        alert.info("Hiện tại đang có một sản phẩm",{position:positions.BOTTOM_CENTER})
        return 
      }
      let check = false
...
export default ChiTietDangXuLy
