...
function EditBooking({ EditData, onHandleEdit, ListTypeBooking, handleChangeTypeBooking }) {
  const [IDBooking, setIDBooking] = React.useState(EditData.id_cash)
  const [NameBooking, setNameBooking] = React.useState(EditData.ten_sp)
  const [Password, setPassword] = React.useState('')
  const [RePassword, setRePassword] = React.useState('')
  const [Address, setAddress] = React.useState('')
  const [PhoneNumber, setPhoneNumber] = React.useState('')
  const [IDTypeBooking, setIDTypeBooking] = React.useState(EditData.id_loai_sp)
  ...
  const onClickSave = async () => {
    try {
      ...
      const res = await fetch(host.WebDashDanhSachBooking, {
        ...
        body: JSON.stringify({ IDBooking, Description, TypeStatus, PriceVND, userName })
      })
  ...
  return (
    <>
      <div>
        ...
        <button type="button" className="btn btn-warning" data-toggle="modal" data-target={`#cash_money_edit${EditData.id_cash}`}
          onClick={() => setTimeout(() => $('.modal-backdrop').remove(), 500)}
        >
          Edit
        </button>
        {/* Modal */}
        <div className="modal fade" id={`cash_money_edit${EditData.id_cash}`} tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title" id="exampleModalLabel">Booking id {EditData.id_cash} </h5>
