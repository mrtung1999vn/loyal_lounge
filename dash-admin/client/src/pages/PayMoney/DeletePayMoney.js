...
<div>
        ...
        <button type="button" className="btn btn-danger" data-toggle="modal" data-target={`#pay_id_delete${EditData.id_cash}`}
          onClick={() => setTimeout(() => $('.modal-backdrop').remove(), 500)}
        >
          Delete
        </button>
        {/* Modal */}
        <div className="modal fade" id={`pay_id_delete${EditData.id_cash}`} tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title" id="exampleModalLabel">Delete PayMoney id {EditData.id_kh} </h5>
                <button type="button" className="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span>
                </button>
              </div>
              <div className="modal-body">
                Do you want to delete PayMoney {Email} (ID: {IDPayMoney}) ?
