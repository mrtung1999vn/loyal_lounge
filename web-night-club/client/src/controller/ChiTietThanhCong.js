...
function ChiTietThanhCong({id_kh,stt,id_don_hang}) {
  ...
  setso_luong_tong(so_luong)
    ...
    const onClickDatHang = async ()=>{
      try{
          ...
          const JsonData = await response.json()
          if(JsonData.status ===1){
              alert.success(" Đơn hàng thành công",{position: positions.BOTTOM_CENTER})
              history.goBack()
          }else{
              alert.error("Đơn hàng thất bại",{position: positions.BOTTOM_CENTER})
          }
...
export default ChiTietThanhCong
