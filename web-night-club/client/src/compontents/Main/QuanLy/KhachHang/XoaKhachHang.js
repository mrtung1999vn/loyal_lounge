...
function XoaKhachHang({ dulieu ,onXoaKhachHang}) {
    ...
    const onClickXoaKhachHang = async () => {
        try {
                var token = await Token.Token()
                const response = await fetch(host.DSKhachHangXoa,{
                    method:"DELETE",
    ...
    return (
        <Fragment>
            <button className="btn btn-danger btn-sm" data-toggle="modal" data-target={`#IDXoaKhachHang${id_kh}`}
                style={{ width: '60px' }}>
            ...
            {/* Modal */}
            <div className="modal fade" id={`IDXoaKhachHang${id_kh}`} tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
