            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                if(token.split("*")[1] === data && checkToken.rowCount > 0){
                    ...
                    const newQuery_THONGBAO = await pool.query(`
                        ...
                        values(N'[Đơn hàng chờ xử lý] ${ten_KH.rows[0].ten_kh}' ,N'Hiện tại đơn hàng mã đơn tạm thời [${key_pr}] khách ${ten_KH.rows[0].ten_kh} chờ xử lý','${timeNowDB}',false,N'Hệ thống',${id_kh})
                    `)
