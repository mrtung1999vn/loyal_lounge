...
import DSKhieuNai from '../../../compontents/Main/TrangThaiDon/KhieuNai/DSKhieuNai'
...
function Form_KhieuNai() {
    //#region setState
    const classes = useStyles();
    const [open, setOpen] = useState(false);
    const [KhieuNai,setKhieuNai] = useState([])
    ...
    const UpdateThemKhieuNai = (e)=>{
        try {
            const newData = [...KhieuNai]
            console.log(e)
            newData.push(e[0])
            LoadingForm()
            setKhieuNai(newData)
    ...
    const onChangePage = async (e)=>{
        var trangthai = "Khiếu nại"
        const token = await Token.Token()
        const response = await fetch(host.DSKhieuNai+`/${e}`,{
            method:"POST",
        ...
        if(JsonData.status === 1 ){
            setKhieuNai(func.DecodeJson_RESPONSE(JsonData.data))
    ...
    const totalPageCallBack = React.useCallback(async ()=>{
        try {
            const response = await fetch(host.TotalPageKhieuNai)
            const JsonData = await response.json()
    ...
    React.useEffect(async ()=>{
        try {
            const token = await Token.Token()
            var trangthai = "Khiếu nại"
            const response = await fetch(host.DSKhieuNai+`/${page}`,{
                method:"POST",
                headers:{"Content-Type" : "application/json"},
                body:JSON.stringify({token,trangthai})
            })
        ...
        <div className="content-wrapper">
            ...
            <div className="content">
                <div className="container-fluid">
                    ...
                    <DSKhieuNai KhieuNai={KhieuNai}
                    totalPage={totalPage}
                    UpdateXoaKhieuNai={UpdateXoaKhieuNai}
                    UpdateSuaKhieuNai={UpdateSuaKhieuNai}
                    page={page}
