...
import DSShopHuy from '../../../compontents/Main/TrangThaiDon/ShopHuy/DSShopHuy'
...
function Form_ShopHuy() {
    //#region setState
    const classes = useStyles();
    const [open, setOpen] = useState(false);
    const [ShopHuy,setShopHuy] = useState([])
    ...
    const UpdateThemShopHuy = (e)=>{
        try {
            const newData = [...ShopHuy]
            console.log(e)
            newData.push(e[0])
            LoadingForm()
            setShopHuy(newData)
    ...
    const onChangePage = async (e)=>{
        var trangthai = "Shop huỷ"
        const token = await Token.Token()
        const response = await fetch(host.DSShopHuy+`/${e}`,{
            method:"POST",
        ...
        if(JsonData.status === 1 ){
            setShopHuy(func.DecodeJson_RESPONSE(JsonData.data))
    ...
    const totalPageCallBack = React.useCallback(async ()=>{
        try {
            ...
            const response = await fetch(host.TotalPageShopHuy)
            const JsonData = await response.json()
    ...
    React.useEffect(async ()=>{
        try {
            const token = await Token.Token()
            var trangthai = "Shop huỷ"
            const response = await fetch(host.DSShopHuy+`/${page}`,{
                method:"POST",
                headers:{"Content-Type" : "application/json"},
                body:JSON.stringify({token,trangthai})
            })
    ...
    const onClickXoaDonThanhCong = async (e)=>{
        try{
            const newData = [...ShopHuy]
        ...
        <div className="content-wrapper">
            ...
            <div className="content">
                <div className="container-fluid">
                    ...
                    <DSShopHuy 
                    onClickXoaDonThanhCong={onClickXoaDonThanhCong}
                    onChangePage={onChangePage}
                    ShopHuy={ShopHuy}
                    totalPage={totalPage}
                    UpdateXoaShopHuy={UpdateXoaShopHuy}
                    UpdateSuaShopHuy={UpdateSuaShopHuy}
                    page={page}
